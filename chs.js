...
var cnPrefix = {
    ...
    "blackberry: ": "黑莓: ",
    "save on refresh / actions: ": "在 刷新/行动 时保存：",
    "leftmost": "最左边",
    "Controls": "控制",
    "Unlocked at ethereal tree level: ": "解锁于空灵树等级: ",
    'Larger field ': '更大的农田 ',
    "flowers ": "花朵 ",
    "mushrooms ": "蘑菇 ",
    "ethereal stinging ": "空灵蛰刺 ",
    "Current bonus at tree level ": "当前加成在树等级 ",
    "Tree's gesture ": "树的姿态 ",
    "Twigs gain ": "树枝增益 ",
    "Next upgrade bonus at tree level ": "下一个升级加成在树等级 ",
    "Production boost for unspent twigs: ": "未使用树枝的产量提升：",
    "": "",
    "": "",
    "": "",
    "": "",
...
var cnRegReplace = new Map([
    ...
    [/^suffixes up to (.+), then eng$/, '后缀到 $1，然后是 工程符号'],
    [/^suffixes up to (.+), then sci$/, '后缀到 $1，然后是 科学计数法'],
    [/^(\d+) days$/, '$1 天'],
    ...
    [/^Best\/hour: (.+) at level (.+), (\d+)h (\d+)m (\d+)s \(previous run: (.+), lvl (\d+), (\d+)h (\d+)m (\d+)s$/, '最好/小时：$1, $2级，$3小时 $4分 $5秒 (之前运行：$6, $7级，$8小时 $9分 $10秒'],
    [/^Twigs\/hour: (.+) \(previous run: (.+), (\d+)h (\d+)m (\d+)s$/, '树枝/小时：$1  (之前运行：$2, $3小时 $4分 $5秒'],
    [/^(.+) seeds, have: (.+) seeds \($/, '$1 种子, 拥有: $2 种子 \('],
    [/^increase ethereal field size to (.+) tiles$/, '将空灵田地大小增加到 $1 格'],
    [/^(.+) seeds, have$/, '$1 种子, 拥有'],
    [/^(.+) \(previous run: (.+) at level (.+)$/, '$1 (上一轮: $2 在等级 $3'],
    [/^ (.+) nuts.$/, '$1 坚果。'],
    ...
    [/^(\d+) day$/, '$1 天'],
    [/^increase basic field size to (\d+)x(\d+) tiles$/, '提高基础地块大小到 $1x$2 格'],
    [/^(\d+) resin, (\d+)$/, '$1 树脂，$2'],
    ...
    [/^([\d\.]+) years$/, '$1 年'],
    [/^field ([\d\.]+)x([\d\.]+)$/, '地块 $1x$2'],
    [/^([\d\.]+)\% of stacks\) \(([\d\.]+)x$/, '$1\% 堆叠\) \($2x'],
    [/^Blackberry ([\d\.]+) \(\+([\d\.]+)$/, '黑莓 $1 \(\+$2'],
    [/^I, consumed: (.+)$/, 'I, 消耗: $1'],
    [/^IX, consumed: (.+)$/, 'IX, 消耗: $1'],
    [/^VI, consumed: (.+)$/, 'VI, 消耗: $1'],
    [/^VII, consumed: (.+)$/, 'VII, 消耗: $1'],
    [/^VIII, consumed: (.+)$/, 'VIII, 消耗: $1'],
    [/^VIV, consumed: (.+)$/, 'VIV, 消耗: $1'],
    [/^VV, consumed: (.+)$/, 'VV, 消耗: $1'],
    [/^([\d\.]+): zinc$/, '$1: 锌'],
    ...
    [/^Grow time: (\d+) minute (\d+) seconds \(base: (\d+) minutes\).$/, '生长时间：$1 分 $2 秒（基础：$3 分钟）。'],
    [/^Grow time: (\d+) seconds \(base: (\d+) minutes\).$/, '生长时间：$1 秒（基础：$2 分钟）。'],
